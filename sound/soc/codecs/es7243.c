...
//#if 0	
	//for(i=0; i<(ES7243_CHANNELS_MAX)/2; i++){
		...
		es7243_write(ES7243_SDPFMT_REG01, 0xCF, i2c_clt[i]); //dsp for tdm mode, DSP-A, 16BIT	
...
//		es7243_write(ES7243_ANACTL0_REG07, 0x00, i2c_clt[i]); //power up adc and analog input
...
/*
		es7243_write(ES7243_MODECFG_REG00, 0x01, i2c_clt[i]);//enter into hardware mode
		es7243_write(ES7243_STATECTL_REG06, 0x18, i2c_clt[i]); //soft reset codec
		es7243_write(ES7243_SDPFMT_REG01, 0x8F, i2c_clt[i]); //dsp for tdm mode, DSP-A, 16BIT	
		es7243_write(ES7243_CLKDIV_REG04, 0x02, i2c_clt[i]); 
		es7243_write(ES7243_MUTECTL_REG05, 0x1a, i2c_clt[i]); 
*/
		if(i == 3) {
...
static int es7243_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_write(codec, ES7243_MODECFG_REG00, 0x01);//enter into hardware mode

	snd_soc_write(codec, ES7243_STATECTL_REG06, 0x18); //soft reset codec
	if(es7243->tdm)
		snd_soc_write(codec, ES7243_SDPFMT_REG01, 0x8F); //dsp for tdm mode, DSP-A, 16BIT
	else
		snd_soc_write(codec, ES7243_SDPFMT_REG01, 0x00); //i2s mode
		
	snd_soc_write(codec, ES7243_LRCDIV_REG02, 0x10); 
	snd_soc_write(codec, ES7243_BCKDIV_REG03, 0x04); 
	snd_soc_write(codec, ES7243_CLKDIV_REG04, 0x02); 
	snd_soc_write(codec, ES7243_MUTECTL_REG05, 0x1a); 
	snd_soc_write(codec, ES7243_ANACTL1_REG08, 0x43);   //enable microphone input and pga gain for 27db
	snd_soc_write(codec, ES7243_ANACTL2_REG09, 0x3F); 
	snd_soc_write(codec, ES7243_STATECTL_REG06, 0x00); 
	snd_soc_write(codec, ES7243_ANACTL0_REG07, 0x00); //power up adc and analog input
	snd_soc_write(codec, ES7243_MUTECTL_REG05, 0x12); 
	/* power on device */
	es7243_set_bias_level(codec, SND_SOC_BIAS_STANDBY);
	//codec->dapm.idle_bias_off = 1;
	//snd_soc_add_codec_controls(codec, es7243_snd_controls,
